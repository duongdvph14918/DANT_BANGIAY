...
public class OrderController {
	...
	@GetMapping("/4MEN/order/checkout")
	public String checkout(Model model) {
		if(useAcc.User()==null) {
			return "redirect:/login";
		}else {
			List<ProductDetail> list = productService.findTop6Img();
...
//	
    @PostMapping("/4MEN/order/checkout/addAdress")
	public String addAddress(Model model, HttpServletRequest request) {
		if(useAcc.User()==null) {
			...
			String nameAdd = request.getParameter("addressOp");
			address.setName(nameAdd);
	...
	@PostMapping("/4MEN/order/checkout")
	@ResponseBody
	public String checkoutPost(Model model, @ModelAttribute("account") Accounts accountKh, HttpServletRequest request) throws IOException{
		...
		if(useAcc.User()==null) {
			...
			System.out.println("maVoucher: "+ maVoucher);
			 ...
			 String vnp_Version = "2.1.0";
				...
				session.setAttribute("kh_diachi", address);
		        if(pttt.equals("1")) {
...
//								Long amountTongg = (long) (order.getTotal() - ((order.getTotal()*vouchers.getValued())/100));
								...
								for (int i = 0; i < lst.size(); i++) {
									...
									if(voucherId.equals(maVoucher2)) {
										stt += 1; 
									}
								}
								if(stt == 0) {
