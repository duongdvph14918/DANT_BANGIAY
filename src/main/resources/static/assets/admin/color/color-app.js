app.controller("color-ctrl", function ($scope, $http) {
    ...
    $scope.timKiem = function () {
        ...
        if (trangThai == "") {
            ...
            $http.get(`/rest/color/timKiem/${name}/${trangThai}`).then(resp => {
                $scope.items = resp.data;
            })
        } else {
            $http.get(`/rest/color/timKiem/${name}/${trangThai}`).then(resp => {
    ...
    $scope.genderStart = function (obj) {
        ...
        if (trangThai == "") {
            ...
            $http.get(`/rest/color/timKiem/${trangThai}`).then(resp => {
                $scope.items = resp.data;
            })
        }
	...
	$scope.create = function () {
		...
		$http.get(`/rest/color/${item.name}`).then(resp =>{
			if (resp.data == '') {
	...
	$scope.update = function () {
		//alert("update ")
		var item = angular.copy($scope.form);
		//alert("colorId " + item.discountId); 
		$http.put(`/rest/color/${item.colorId}`, item).then(resp => {
			var index = $scope.items.findIndex(p => p.colorId == item.colorId);
	...
	$scope.delete = function (item) {
		...
		if (t == false) {
			...
			//alert("có xóa");
			/*$http.delete(`/rest/color/${item.colorId}`).then(resp => {
				var index = $scope.items.findIndex(p => p.colorId == item.colorId);
			...
			$http.put(`/rest/color/delete/${item.colorId}`, item).then(resp => {
				var index = $scope.items.findIndex(p => p.colorId == item.colorId);
				$scope.items[index] = item;
				alert("xóa thành công");
				$scope.initialize();
	...
	$scope.check = function (name){
		$http.get(`/rest/color/${name}`).then(resp =>{
			if (resp.data == '') {
