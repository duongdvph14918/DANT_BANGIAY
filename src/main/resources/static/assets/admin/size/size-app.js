app.controller("size-ctrl", function ($scope, $http) {
	...
	$scope.update = function () {
		//alert("update ")
		var item = angular.copy($scope.form);
		//alert("sizeId " + item.discountId); 
		var sizeName = document.getElementById("sizeName").value;
		if(sizeName.trim() =="" || sizeName ==null){
			alert("Tên Size không được để trống");
			return false;
		}
		$http.put(`/rest/size/${item.sizeId}`, item).then(resp => {
	...
	$scope.delete = function (item) {
		...
		if (t == false) {
			...
			//alert("có xóa");
			/*$http.delete(`/rest/size/${item.sizeId}`).then(resp => {
				var index = $scope.items.findIndex(p => p.sizeId == item.sizeId);
			...
			$http.put(`/rest/size/delete/${item.sizeId}`, item).then(resp => {
				var index = $scope.items.findIndex(p => p.sizeId == item.sizeId);
				$scope.items[index] = item;
				alert("xóa thành công");
				$scope.initialize();
